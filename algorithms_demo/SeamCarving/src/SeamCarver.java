...
public class SeamCarver {
	...
	private int marginOfError = 5; // removeRegion will terminate when
									// this many pixels are still left
									// in region targeted for removal
	private final double removalEnergy = -borderEnergy;
	private boolean removeRegion = false;
	private int removedFromRegion = 0;
	...
	private void removeSeam(int[] a, int W, int H) {
		...
		for (int y = 0; y < H; y++) {
			...
			if (removeRegion) {
				if (toRemove[y][c])
					removedFromRegion++;
				toRemove[y] = removeElement(toRemove[y], c);
			}
			...
			if (removeRegion) {
				if (c < W)
					if (toRemove[y][c])
						energy[y][c] = removalEnergy;
				if (c > 0)
					if (toRemove[y][c - 1])
						energy[y][c - 1] = removalEnergy;
			}
		}
	}
	...
	public void removeRegion(int x0, int y0, int x1, int y1) {
		// delete rectangular region, from top left pixel (x0, y0)
		// to bottom right pixel (x1, y1), inclusive
		if (x0 < 0 || y0 < 0 || x1 >= W || y1 >= H || x0 > x1 || y0 > y1)
			throw new IllegalArgumentException(
					"Region to delete must be inside image");
		...
		for (int y = y0; y <= y1; y++) {
			for (int x = x0; x <= x1; x++) {
				toRemove[y][x] = true;
				if (!transposed)
					energy[y][x] = removalEnergy;
				else
					energy[x][y] = removalEnergy;
			}
		}
		if (vertical) {
			if (transposed)
			...
			toRemove = transpose(toRemove);
			if (!transposed)
				transposeMatrices();
		}

		removedFromRegion = 0; // number of target elements removed so far
		removeRegion = true;
		while (removedFromRegion < (x1 - x0 + 1) * (y1 - y0 + 1)
				- marginOfError) {
			int[] seam;
			if (vertical) {
	...
	private boolean[][] transpose(boolean[][] in) {
		if (in.length == 0)
			throw new UnsupportedOperationException("Array underflow");
		if (in[0].length == 0)
			throw new UnsupportedOperationException("Array underflow");
	...
	private boolean[] removeElement(boolean[] in, int element) {
		if (in.length == 0)
			throw new UnsupportedOperationException("Array underflow");
		boolean[] out = new boolean[in.length - 1];
		System.arraycopy(in, 0, out, 0, element);
		System.arraycopy(in, element + 1, out, element, out.length - element);
		return out;
	}
