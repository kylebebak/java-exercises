 ...
 * Modified so that every node has a default value, which makes initializing the
 * trie much faster
 * 
...
public class TSTprefix<Value> {
	private int N; // size
	private Node root; // root of TST
	private Value def;

	public TSTprefix(Value def) {
		this.def = def;
	}
	...
	public void collect(Node x, String prefix, int i, String pat,
			Queue<String> q) {
		if (x == null)
			return;
		char c = pat.charAt(i);
	...
	public static void main(String[] args) {
		...
		TSTprefix<Integer> st = new TSTprefix<Integer>(0);
