...
//			StdOut.println(i);
		...
		for (int i = 0; i < 1000; i++) {
			...
			if (StdRandom.uniform(10) > 7) d.addLast(input);
			else d.addFirst(input);
		}
		
		for (int i = 0; i < 1000; i++) {
			if (StdRandom.uniform(2) < 8) StdOut.println(d.removeLast());
			else StdOut.println(d.removeFirst());
		}
		
		for (int i = 0; i < 1000; i++) {
			int input = StdRandom.uniform(50);
			if (StdRandom.uniform(2) < 8) d.addLast(input);
			else d.addFirst(input);
		}
		
		for (int i = 0; i < 980; i++) {
			if (StdRandom.uniform(2) > 7) StdOut.println(d.removeLast());
			else StdOut.println(d.removeFirst());
		}
