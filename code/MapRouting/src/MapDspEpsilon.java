...
public class MapDspEpsilon {
	...
	private final double epsilon = .00000000005; //5E-11, 1E-12 won't work
	
	/**
	 * If s = -1 is passed, the constructor simply initializes the distTo and
	 * edgeTo arrays, ready for calls to shortestPath
	 */
	public MapDspEpsilon(EdgeWeightedDigraph G, Point2D[] points, int s) {
	...
	private void relax(DirectedEdge e, int d) {
		int v = e.from(), w = e.to();
		if (distTo[w] > epsilon + distTo[v] + e.weight()
